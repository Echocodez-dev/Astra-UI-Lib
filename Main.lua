...
local requestsDisabled = true --getgenv and getgenv().DISABLE_AstraLib_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.672"
local AstraLibFolder = "AstraLib"
local ConfigurationFolder = AstraLibFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		AstraLibOpen = {Type = 'bind', Value = 'K', Name = 'AstraLib Keybind'},
		-- buildwarnings
		-- AstraLibprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(AstraLibFolder) then
				if isfile and isfile(AstraLibFolder..'/settings'..ConfigurationExtension) then
					file = readfile(AstraLibFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"AstraLibOpen":{"Value":"K","Type":"bind","Name":"AstraLib Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"AstraLib Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('AstraLib had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("AstraLib", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify AstraLib developers at sirius.menu/discord.")
				end
			end)
...
local AstraLib = useStudio and script.Parent:FindFirstChild('AstraLib') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if AstraLib:FindFirstChild('Build') and AstraLib.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('AstraLib | Build Mismatch')
		print('AstraLib may encounter issues as you are running an incompatible interface version ('.. ((AstraLib:FindFirstChild('Build') and AstraLib.Build.Value) or 'No Build') ..').\n\nThis version of AstraLib is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, AstraLib = AstraLib, useStudio and script.Parent:FindFirstChild('AstraLib') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

AstraLib.Enabled = false

if gethui then
	AstraLib.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(AstraLib)
	AstraLib.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	AstraLib.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == AstraLib.Name and Interface ~= AstraLib then
			Interface.Enabled = false
			Interface.Name = "AstraLib-Old"
		end
	end
elseif not useStudio then
...
if AstraLib.AbsoluteSize.X < minSize.X and AstraLib.AbsoluteSize.Y < minSize.Y then
...
local Main = AstraLib.Main
local MPrompt = AstraLib:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = AstraLib:FindFirstChild('Drag')
...
AstraLib.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/AstraLib/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = AstraLib.Notifications

local SelectedTheme = AstraLibLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = AstraLibLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	AstraLib.Main.BackgroundColor3 = SelectedTheme.Background
	AstraLib.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	AstraLib.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	AstraLib.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	AstraLib.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	AstraLib.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	AstraLib.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		AstraLib.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		AstraLib.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(AstraLib:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("AstraLib | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("AstraLib | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('AstraLib had an issue decoding the configuration file, please try delete the file and reopen AstraLib.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(AstraLibLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("AstraLib | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--AstraLibLibrary:Notify({Title = "AstraLib Flags", Content = "AstraLib was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(AstraLibLibrary.Flags) do
...
function AstraLibLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('AstraLib | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			AstraLibLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show AstraLib'.", Duration = 7, Image = 4400697855})
		else
			AstraLibLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.AstraLibOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(AstraLibFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('AstraLib Settings', 0, true)

	if TabList['AstraLib Settings'] then
		TabList['AstraLib Settings'].LayoutOrder = 1000
	end

	if Elements['AstraLib Settings'] then
		Elements['AstraLib Settings'].LayoutOrder = 1000
	end
...
function AstraLibLibrary:CreateWindow(Settings)
	if AstraLib:FindFirstChild('Loading') then
		if getgenv and not getgenv().AstraLibCached then
			AstraLib.Enabled = true
			AstraLib.Loading.Visible = true

			task.wait(1.4)
			AstraLib.Loading.Visible = false
		end
	end

	if getgenv then getgenv().AstraLibCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				AstraLibLibrary:Notify({Title = 'Build Mismatch', Content = 'AstraLib may encounter issues as you are running an incompatible interface version ('.. ((AstraLib:FindFirstChild('Build') and AstraLib.Build.Value) or 'No Build') ..').\n\nThis version of AstraLib is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(AstraLibFolder) then
		makefolder(AstraLibFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "AstraLib"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "AstraLib Interface Suite" then
		LoadingFrame.Version.Text = "AstraLib UI"
	end
	...
	if not Settings.DisableAstraLibPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				AstraLibLibrary:Notify({
					Title = "AstraLib Interface",
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(AstraLibFolder.."/Discord Invites") then
			makefolder(AstraLibFolder.."/Discord Invites")
		end

		if isfile and not isfile(AstraLibFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(AstraLibFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"AstraLib RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(AstraLibFolder.."/Key System") then
			makefolder(AstraLibFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("AstraLib | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AstraLib specific development.')
				end
			end
		end
		...
		if isfile and isfile(AstraLibFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(AstraLibFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(AstraLibFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						AstraLibLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AstraLib | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AstraLib specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= AstraLibLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					AstraLibLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			AstraLib.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("AstraLib | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AstraLib specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					AstraLibLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("AstraLib | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AstraLib specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					AstraLibLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("AstraLib | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AstraLib specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					AstraLibLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= AstraLibLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AstraLib | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AstraLib specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						AstraLibLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("AstraLib | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with AstraLib specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					AstraLibLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			AstraLibLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			AstraLibLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('AstraLib had an issue creating settings.') end
...
function AstraLibLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function AstraLibLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function AstraLibLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['AstraLib Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.AstraLibOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function AstraLibLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				AstraLibLibrary:Notify({Title = "AstraLib Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			AstraLibLibrary:Notify({Title = "AstraLib Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('AstraLib Configurations Error | '..tostring(result))
			AstraLibLibrary:Notify({Title = "AstraLib Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = AstraLibLibrary:CreateWindow({
		Name = "AstraLib Example Window",
		LoadingTitle = "AstraLib Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using AstraLib may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like AstraLib to get the key from
	...
	--AstraLibLibrary:Notify({Title = "AstraLib Interface", Content = "Welcome to AstraLib. These - are the brand new notification design for AstraLib, with custom sizing and AstraLib calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(AstraLibLibrary.Theme) do
...
task.delay(4, function()
	AstraLibLibrary.LoadConfiguration()
